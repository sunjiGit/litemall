...
CREATE TABLE `litemall_account` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uniq_user_id_acc_type` (`user_id`,`account_type`),
...
CREATE TABLE `litemall_account_flow` (
  `id` int(11) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键',
  `uniq_flow_id` varchar(64) DEFAULT NULL COMMENT '唯一流水号',
  `user_id` int(11) DEFAULT NULL COMMENT '用户ID\n',
  `account_id` int(11) DEFAULT NULL COMMENT '账户ID',
  `type` varchar(16) DEFAULT NULL COMMENT '流水类型：RECHARGE-充值，CONSUME-消费，RED_ENVELOPE-红包充值',
